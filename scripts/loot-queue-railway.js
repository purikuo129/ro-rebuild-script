...
// Railway-hosted variant of loot-queue-poc.js.
// It deliberately remains a separate file: local use continues to use
// loot-queue-poc.js on ws://127.0.0.1:8787 without any Railway settings.
//
// Required Railway variable: LOOT_QUEUE_TOKEN=<a long random secret>
// Browser URL: wss://<your-domain>.up.railway.app/?token=<same-secret>

const crypto = require('crypto');
const http = require('http');

const port = Number(process.env.PORT || process.argv[2]) || 8787;
const host = process.env.LOOT_QUEUE_HOST || '0.0.0.0';
const requiredToken = String(process.env.LOOT_QUEUE_TOKEN || '').trim();
const jobs = new Map(); // group:map:dropId -> job
const clients = new Set();

if (!requiredToken) {
  console.error('LOOT_QUEUE_TOKEN is required. Refusing to expose an unauthenticated public queue.');
  process.exit(1);
}
...
function expireJobs() {
  ...
  for (const [id, job] of jobs) {
    if (job.expiresAt <= now) {
      jobs.delete(id); broadcast(job.group, { type: 'expired', id }); log(`Expired ${id}`); continue;
    }
    if (job.state === 'claimed' && job.leaseUntil <= now) {
      job.state = 'open'; job.claimedBy = null; job.claimToken = null; job.leaseUntil = 0;
      broadcast(job.group, { type: 'available', job }); log(`Lease expired; reopened ${id}`);
    }
  }
}
function handle(client, message) {
  ...
  if (message.type === 'hello') {
    ...
    send(client, { type: 'welcome', service: 'loot-queue-railway', group: client.group });
  ...
  if (message.type === 'claim') {
    ...
    send(client, { type: 'claimed', job, claimToken: job.claimToken }); log(`Claim ${job.id} by ${client.clientId}`);
    return;
  }
  if (job.claimedBy !== client.clientId || job.claimToken !== message.claimToken) return send(client, { type: 'error', reason: 'claim mismatch' });
  if (message.type === 'renew') { job.leaseUntil = Date.now() + 20_000; return; }
  if (message.type === 'ack' || message.type === 'discard') {
    jobs.delete(job.id);
    const type = message.type === 'ack' ? 'completed' : 'discarded';
    broadcast(client.group, { type, id: job.id, itemId: job.itemId, itemName: job.itemName, reason: String(message.reason || '') });
    ...
    log(`${message.type === 'ack' ? 'Complete' : 'Discard'} ${job.id}${message.reason ? ': ' + message.reason : ''}`);
    return;
  }
  if (message.type === 'nack') {
    job.state = 'open'; job.claimedBy = null; job.claimToken = null; job.leaseUntil = 0;
    broadcast(client.group, { type: 'available', job }); log(`Released ${job.id}: ${message.reason || 'no reason'}`);
  }
}
...
const server = http.createServer((req, res) => {
  if (req.url === '/health') {
    const openJobs = [...jobs.values()].filter(job => job.state === 'open').length;
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ ok: true, service: 'loot-queue-railway', connections: clients.size, jobs: jobs.size, openJobs }));
    return;
  }
  res.writeHead(426, { 'Content-Type': 'text/plain' });
  res.end('WebSocket upgrade required; use /health for health checks.\n');
});

server.on('upgrade', (req, socket) => {
  let token = '';
  try { token = new URL(req.url || '/', `http://${req.headers.host || 'localhost'}`).searchParams.get('token') || ''; } catch (_) {}
  ...
  clients.add(client);
  send(client, { type: 'hello', service: 'loot-queue-railway', connections: clients.size });
  socket.on('data', chunk => parseFrames(client, chunk));
  socket.on('close', () => { clients.delete(client); log(`Client disconnected${client.clientId ? `: ${client.clientId}` : ''} (${clients.size} connected)`); });
  socket.on('error', () => {});
});

server.listen(port, host, () => {
  log(`Railway loot queue ready: ${host}:${port}`);
  log('Health check: /health · Queue is memory-only and clears after a restart/deploy.');
});
