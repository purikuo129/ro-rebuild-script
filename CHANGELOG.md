...
- แก้ Combat GAT ที่ส่ง waypoint เดิมซ้ำเมื่ออยู่ชิดขอบระยะ Attack-follow (เช่น 10.8/10 หรือ 4/2) โดย handoff เป็น Attack-follow ตาม GAT arrival tolerance เดิมก่อน timer วาร์ปตัด flow
- แก้ Auto-Loot ที่บางรอบปล่อยของจากมอนที่เพิ่งฆ่าเมื่อ packet EXP มาหลัง packet ตาย: บันทึกพิกัดมอนจาก packet ตายก่อนล้างเป้า จึงใช้ค่าเช็คระยะพิกัดมอนที่ฆ่าได้ตามปกติ

## [2.0.0-rc.2] — 2026-09-08

### Fixed

- แก้ Combat fallback เมื่อ GAT ยังไม่พร้อมส่ง `MOVE` เพียงครั้งเดียวแล้วรอพิกัดจน timer วาร์ปครบ; ตอนนี้ retry การเดินทุก 800ms ตาม cadence เดิมของ GAT ระหว่างนับเวลาเข้าหามอนรวม
...
- เพิ่มค่าเวลาเข้าหามอนรวมก่อนวาร์ปไปหาในหน้า Combat ค่าเริ่มต้น 5 วินาที และตั้ง `0` เพื่อปิด timed warp
- เพิ่มสถานะเวลาเข้าหาเป้าและเวลาที่เหลือก่อนวาร์ปใน Bot Activity

### Fixed

- แยก Attack probe และ GAT progress timeout ให้ใช้ retry/replan เท่านั้น ไม่ชิงสั่งวาร์ประหว่างที่ Bot ยังเดินเข้าหามอน
- แก้ A* simplification ที่ทำเส้นทางตรงหายเหลือเพียงจุดเริ่ม และแก้การปัดพิกัดตรงขอบ 15 ช่องที่เลือกตำแหน่งเดิมเป็นปลายทาง
- ล้าง Movement Planner, GAT path และ target ก่อนวาร์ป พร้อมใช้ teleport guard ป้องกัน MOVE เก่าทับคำสั่งวาร์ป

## [1.3.1] — 2026-09-06

### Added

- เพิ่มช่องตั้งเวลาที่ไม่พบมอนก่อนวาร์ปสุ่มในหมวด Combat โดยใช้ค่า `noMonsterWarpSec` เดิมและบันทึกลง Profile
- เพิ่มช่องตั้งเวลารอหลังจบ Combat/เก็บของเสร็จในหมวด Combat รองรับ `0` เพื่อไม่รอ

### Changed

- เปลี่ยนค่าเริ่มต้นของ Post-combat delay จาก 800ms เป็น 200ms; Profile เดิมยังคงค่าที่บันทึกไว้และปรับผ่าน UI ได้
